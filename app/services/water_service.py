...
async def log_water(user_id: str, request: LogWaterRequest) -> WaterConsumption:
    ...
    intake_time = request.intake_time or datetime.now()
    ...
    await db_client.db.activities.update_one(
        ...
        {
            "$inc": {"totalWaterMl": request.amount_ml},
            "$set": {"updatedAt": datetime.now()}
        }
    )
    
    new_log["_id"] = result.inserted_id
    return WaterConsumption(**new_log)
...
async def update_water_log(user_id: str, log_id: str, request: UpdateWaterLogRequest) -> WaterConsumption:
    ...
    amount_diff = request.amount_ml - old_log["amountMl"]
    ...
    if request.intake_time:
        update_data["dateAndTime"] = request.intake_time
